...
const Login = () => {
    ...
    return (
        <div className={`min-h-screen flex justify-center items-center ${bgLoaded ? '' : 'bg-white'}`} style={bgLoaded ? { backgroundImage: `URL(${MainScreen})`, backgroundPosition: "center", backgroundSize: "cover" } : {}}>
            <div className='bg-white/75 w-full sm:w-2/3 md:w-1/2 lg:w-1/3 p-4 md:rounded-xl shadow-2xl flex flex-col items-center'>
                {RMILogo ? (
                    ...
                    <img src={ReduceRMILogo} alt="Readyman, Inc" className='w-24 h-24 mb-4' />
                )}
                ...
                <form className='flex flex-col w-full px-4' onSubmit={validateLogin}>
                    <div className='flex flex-row gap-2 items-center mb-4'>
                        <UserRound />
                        <input type="text" className='flex-1 rounded-lg border p-2 bg-neutral-200'
                            value={user_name} onChange={(item) => usernameupdate(item.target.value)} placeholder="Username" />
                    </div>
                    <div className='flex flex-row gap-2 items-center mb-4'>
                        <KeyRound />
                        <input className='flex-1 rounded-lg border p-2 bg-neutral-200'
                            type="password" value={password} onChange={(item) => passwordupdate(item.target.value)} placeholder="Password" />
                    </div>
                    <button className='bg-blue-600 text-white rounded-lg p-2' type='submit'>
                        Login
                    </button>
                </form>
            </div>
