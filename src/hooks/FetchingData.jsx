import { BriefcaseBusiness, HandCoins, MapPin, SquareGanttChart } from 'lucide-react';
...
const FetchingData = ({ filterCriteria }) => {
    ...
    return (
        <div className="w-w90p h-h90p overflow-hidden mx-auto">
            <div className="overflow-y-auto max-h-full">
                <ul className="space-y-2 divide-white shadow-lg">
                    {filteredJobs.map((item, index) => (
                        <li key={index} className="flex flex-col p-4 border rounded-xl shadow-lg bg-white">
                            ...
                            <div className='flex flex-wrap gap-4 py-2 md:pl-8 pb-4 border-b-2 border-b-sky-200'>
                                <div className='flex gap-2 items-center'>
                                    ...
                                    <span className='text-sm sm:text-base'>
                                        {
                                            Intl.NumberFormat('fil-PH', { style: 'currency', currency: 'PHP' }).format(item.min_salary)} - {Intl.NumberFormat('fil-PH', { style: 'currency', currency: 'PHP' }).format(item.max_salary)
                                        }
                                    </span>
                                </div>
                                <div className='flex gap-2 items-center'>
                            ...
                            <div className='flex justify-between items-center pt-2 border-t-2 border-t-sky-200'>
                                ...
                                <select name="jobstatus" id="jobstatus" className='border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'>
                                    <option value={item.status} defaultValue={true}>{item.status.toUpperCase()}</option>
